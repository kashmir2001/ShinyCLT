...
ui <- fluidPage(
  tags$style(HTML("
    body, label, input, button, select, textarea {
      font-size: 16px !important; /* Force font size across the app */
    }
  ")),
  # App title ----
  titlePanel("A Deeper Exploration of the Central Limit Theorem through the K-S Statistic (DECKS)"),
      ...
      tabsetPanel(type = "tabs",
                   tabPanel("The Basics",
                            ...
                            HTML(paste(
                              ...
                              (which is what you are probably familiar with) and the Lyapunov CLT (which relaxes the assumption of
                              identicalness- you can learn more in the Lyapunov CLT tab!)")),
                  ...
                  tabPanel("Classic CLT", 
                           ...
                           fluidRow(
                             column(align = "center", width = 12, offset = 4,
                                    HTML(paste(
                                      ...
                                      "The plots above show 3 samples of your chosen distribution and parameters. The vertical line in each plot highlights the mean of each sample.",
                                      '</p>'
                                    ))
                             )
                           ),
                           ...
                           HTML(paste("The simulation below computes the theoretical K-S statistic of your chosen distribution and parameters,
                                      ...
                                      and the table resets after 6 columns.")),
                           div(style = "height:50px"), #gives space between text and the panel below
                           sidebarPanel(
...
server<-function(input, output){
  ...
  output$normalc<-renderPlot({
    ...
    plot(myecdf,xlab="",ylab="Probability",main="",
         cex.main = 1.5,  # Increase title font size
         cex.lab = 1.5,   # Increase axis label font size
         cex.axis = 1.2   ) #plot simulated distribution's CDF 
    lines(seq,pnorm(seq,m,s),col="deepskyblue3",lwd=5) #plot theoretical normal CDF
    
    multicolor.title(c("Simulated Normal CDF and ","Normal CDF ", "with", " K-S ",ks), c("black","deepskyblue3","black","red","red"),
                     cex = 1.5) #colors correspond to features on graph
  ...
  output$normalt<-renderPlot({
    ...
    plot(seq,pnorm(seq,m,s),xlab="",ylab="Probability",main="",pch=1,cex=1,ylim=c(0,1),
         cex.main = 1.5,  # Increase title font size
         cex.lab = 1.5,   # Increase axis label font size
         cex.axis = 1.2) #plot simulated distribution's theoretical CDF 
  ...
  output$classicclt <- renderPlot({
    ...
    hist(data1, xlab = "", col = "magenta4", main = "",xlim=c(floor(xmin),ceiling(xmax)),
         cex.main = 1.7,  # Title font size
         cex.lab = 1.7,   # Axis label font size
         cex.axis = 1.7) #we want the limits to be integers (so it's clean) that still capture the smallest and largest values so we use floor and ceiling
  ...
  observeEvent(input$update, {
    ...
    col_counter$index <- col_counter$index + 1 #increase column counter by 1 to represent next column
      ...
      table_data$data[, col_name] <- if (input$cltd == "Normal") {
        ...
        # }
        normal.ks <- function(nn) {
          # Generate a matrix: 10000 rows, each with nn samples
          vals <- rnorm(10000 * nn, mean = m, sd = s)
          mat <- matrix(vals, nrow = 10000, ncol = nn)
          
          # Compute row means — one mean per simulation
          means <- apply(mat, 1, mean)
          
          # Use ks.test on the vector of means
          ks <- ks.test(means, pnorm, m, sqrt(s^2 / nn))$statistic
          ks <- round(ks, 3)
          return(ks)
        }
        ks0<-normal.ks(1)
        ...
        fun.ks <- function(nn) {
          ...
          # Use ks.test on the vector of means
          ks = ks.test(means, pnorm, (r*(1-p))/p, sqrt(((r*(1-p))/(p^2))/nn))$statistic
          ks <- round(ks, 3)
          return(ks)
        }
        ks0<-fun.ks(1)
        ...
        beta.ks <- function(nn) {
          ...
          # Use ks.test on the vector of means
          ks=ks.test(means, pnorm, a/(a+b),sqrt(((a*b)/((a+b)^2*(a+b+1)))/nn))$statistic
          ks <- round(ks, 3)
          return(ks)
        }
        ks0<-beta.ks(1)
        ...
        fun.ks <- function(nn) {
          ...
          # Use ks.test on the vector of means
          ks=ks.test(means, pnorm, .5*(a+b),sqrt(((1/12)*(b-a)^2)/nn))$statistic
          ks <- round(ks, 3)
          return(ks)
        }        
        ...
        pois.ks <- function(nn) {
          ...
          # Use ks.test on the vector of means
          ks=ks.test(means, pnorm, l, sqrt(l/nn))$statistic
          ks <- round(ks, 3)
          return(ks)
        }      
        ...
        geom.ks <- function(nn) {
          ...
          # Use ks.test on the vector of means
          ks=ks.test(means, pnorm, (1-p)/p, sqrt(((1-p)/p^2)/nn))$statistic
          ks <- round(ks, 3)
          return(ks)
        }   
        ...
        exp.ks <- function(nn) {
          ...
          # Use ks.test on the vector of means
          ks=ks.test(means, pnorm, 1/l, (1/l)/sqrt(nn))$statistic
          ks <- round(ks, 3)
          return(ks)
        }   
        ...
        hg.ks <- function(nn) {
          ...
          # Use ks.test on the vector of means
          ks=ks.test(means, pnorm, t*(w/(w+b)), sqrt((t*(w/(w+b))*(1-(w/(w+b)))*((w+b-t)/(w+b-1)))/nn))$statistic
          ks <- round(ks, 3)
          return(ks)
        }   
        ...
        bin.ks <- function(nn) {
          ...
          # Use ks.test on the vector of means
          ks = ks.test(means, pnorm, t*p, sqrt((t*p*(1-p))/nn))$statistic
          ks <- round(ks, 3)
          return(ks)
        }   
        ...
        c.ks <- function(nn) {
          ...
          # Use ks.test on the vector of means
          ks=ks.test(means, pnorm, l, sqrt(s^2/nn))$statistic
          ks <- round(ks, 3)
          return(ks)
        }   
        ks0<-c.ks(1)
    ...
    output$classicplot <- renderPlot({
      ...
      if (ncol(table_data$data)>1) { #if there are >1 columns in the table:
        ...
        par(mar = c(5.1, 4.1, 4.1, 24))  # Reduced the right margin to 6
        ...
        legend("topright", 
               inset = c(-0.3, 0),  # Shift legend to the right (negative x inset)
               legend = colnames(table_data$data)[1:ncol(table_data$data)], 
               col = colors[1:ncol(table_data$data)], 
               pch = 19, 
               cex = 1.7, 
               xpd = TRUE)        
      }
